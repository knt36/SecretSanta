...
$location_file = $target_dir . rand() . "_" . $_SESSION[$webName]['idsecretSanta'] . "." . $imageFileType ;
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $location_file)) {
		...
		$sessionUser = $_SESSION[$webName][idsecretSanta];
		echo("the selected user is: " . $selectedUserID);
		$result = mysqli_query($con,"UPDATE secretSanta SET avatar = '$storedLocation' WHERE idsecretSanta = '$sessionUser';");
        var_dump($result);
